...
const GoalCard = ({ goal, isBalanceShow, onBalanceShow }: any) => {
  ...
  return (
    <View
      ...
      className="w-[48%] p-4 h-40 bg-[#FDF4FF] relative rounded-3xl mb-5 flex justify-between"
    >
      {/* icon and name */}
      <View>
        <View className="flex flex-row items-start justify-between">
          <View
            ...
            className="rounded-full justify-center items-center h-[50px] w-[50px]"
          >
            <Text className="text-xl">{goal.icon}</Text>
          </View>
          <Text className="font-bold" style={{ color: goal.accentColor }}>
            {`${percentage.toFixed(0)}%`}
          </Text>
        </View>
        <Text className="text-xl font-bold mt-2">{goal.name}</Text>
      </View>
      {/* balance and bar */}
      <View className="gap-1.5">
        <View className="flex flex-row items-end">
          <Text className="font-bold">Rp{formatAmount(goal.current)}</Text>
